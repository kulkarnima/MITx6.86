    ...
    if np.random.random() <= epsilon:
        ...
        for action in range(NUM_ACTIONS):
            for object in range(NUM_OBJECTS):
                q_val = q_func[state_1,state_2,action,object]
                if q_val > maxq:
                    maxq = q_val
                    action_index,object_index = action , object
...
def tabular_q_learning(q_func, current_state_1, current_state_2, action_index,
    ...
    if terminal : 
        ...
        q_next = q_func[next_state_1,next_state_2,:,:]
    ...
    q_func[current_state_1,current_state_2,action_index,
           object_index] = (1-ALPHA) * q_value + ALPHA *(reward + GAMMA * maxq_next)    
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        # TODO Your code here
        curr_room_desc_id = dict_room_desc[current_room_desc]
        curr_quest_desc_id = dict_quest_desc[current_quest_desc]
        (action_index, object_index) = epsilon_greedy(curr_room_desc_id,
                                                      curr_quest_desc_id,
                                                      q_func, epsilon)
        ...
        if for_training:
            ...
            tabular_q_learning(q_func, curr_room_desc_id, curr_quest_desc_id, 
                               action_index, object_index, reward, next_room_desc_id,
                               next_quest_desc_id, terminal)
        if not for_training:
            # update reward
            # TODO Your code here
